...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, InfoAlertButton> : ModifyBase<ModifyDerive<Der, InfoAlertButton>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x4c9214, Default, InfoAlertButton, create, gd::string, gd::string, float)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x4c94b0, Default, InfoAlertButton, init, gd::string, gd::string, float)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x4c9554, Default, InfoAlertButton, activate, )
		}
	};
}
