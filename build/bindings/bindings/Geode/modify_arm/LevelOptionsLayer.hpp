...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, LevelOptionsLayer> : ModifyBase<ModifyDerive<Der, LevelOptionsLayer>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x214280, Default, LevelOptionsLayer, valueDidChange, int, float)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x214298, Default, LevelOptionsLayer, getValue, int)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x213c34, Default, LevelOptionsLayer, setupOptions, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x2142b4, Default, LevelOptionsLayer, didToggle, int)
		}
	};
}
