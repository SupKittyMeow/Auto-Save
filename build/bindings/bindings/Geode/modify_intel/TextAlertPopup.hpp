...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, TextAlertPopup> : ModifyBase<ModifyDerive<Der, TextAlertPopup>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x2ab0c0, Default, TextAlertPopup, create, gd::string, float, float, int, gd::string)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x2ab2e0, Default, TextAlertPopup, init, gd::string, float, float, int, gd::string)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x2ab5f0, Default, TextAlertPopup, setLabelColor, cocos2d::ccColor3B)
		}
	};
}
