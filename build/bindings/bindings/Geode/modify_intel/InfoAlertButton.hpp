...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, InfoAlertButton> : ModifyBase<ModifyDerive<Der, InfoAlertButton>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x57cca0, Default, InfoAlertButton, create, gd::string, gd::string, float)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x57cf20, Default, InfoAlertButton, init, gd::string, gd::string, float)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x57cfc0, Default, InfoAlertButton, activate, )
		}
	};
}
