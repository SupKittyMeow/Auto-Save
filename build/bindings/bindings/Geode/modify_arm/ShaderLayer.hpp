...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, ShaderLayer> : ModifyBase<ModifyDerive<Der, ShaderLayer>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x356bf8, Default, ShaderLayer, performCalculations, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x350c14, Default, ShaderLayer, setupShader, bool)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x355f40, Default, ShaderLayer, updateMotionBlurSpeedX, float, float)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x355f74, Default, ShaderLayer, updateMotionBlurSpeedY, float, float)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x352250, Default, ShaderLayer, update, float)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x350b50, Default, ShaderLayer, init, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x356eb0, Default, ShaderLayer, visit, )
		}
	};
}
