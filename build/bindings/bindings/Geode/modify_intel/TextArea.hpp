...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, TextArea> : ModifyBase<ModifyDerive<Der, TextArea>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x320310, Default, TextArea, create, gd::string, char const*, float, float, cocos2d::CCPoint, float, bool)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x320680, Default, TextArea, setString, gd::string)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x3210f0, Default, TextArea, update, float)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x320f40, Default, TextArea, setOpacity, unsigned char)
		}
	};
}
